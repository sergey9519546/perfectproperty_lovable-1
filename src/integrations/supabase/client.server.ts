...
function getEffectiveAdminKey(): string {
  ...
  if (serviceKey && !serviceKey.startsWith("sbp_")) {
    return serviceKey;
  }
  ...
  if (publishableKey) {
    if (serviceKey?.startsWith("sbp_")) {
      console.warn(
        "[Supabase] SUPABASE_SERVICE_ROLE_KEY is a Personal Access Token (sbp_...), falling back to publishable key for PostgREST operations",
      );
    }
